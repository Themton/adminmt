...
export async function exportJntExcel(orders, filename, profile, filterInfo) {
  ...
  orders.forEach((o, i) => {
    ...
    // รายละเอียดพัสดุ = หมายเหตุ (ถ้าไม่มีหมายเหตุ ใช้กลุ่มสินค้า/ชื่อเพจแทน)
    const rem = String(o.remark || '').trim()
    r.getCell(COL.item).value = rem || pgMap[(o.sales_channel || '').trim()] || (o.sales_channel || '').trim() || 'สินค้า'
